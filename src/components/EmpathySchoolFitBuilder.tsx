...
type PriorityKey = "calm" | "community" | "gentle-structure" | "academic-weight" | "unsure";
...
type SolveKey = "rhythm" | "belonging" | "learning-fit" | "everything" | "not-sure";
...
function buildResult(
  ...
  const nextLinks: LinkItem[] = [
    ...
    { title: "Fit notes sheet", href: "/resources/empathy-school-fit-notes-sheet" },
  ];
  ...
  if (stage === "older") {
    ...
    nextLinks.push({ title: "Ask about school fit", href: buildContactHref("Empathy School fit", { from: "/empathy-school-fit" }) });
  }
  ...
  if (move === "school-first") {
    ...
    nextLinks.unshift({ title: "Should Empathy School anchor the move?", href: "/guides/how-to-know-if-empathy-school-should-anchor-your-move" });
  }
  ...
  if (move === "test-stay") {
    ...
    nextLinks.push({ title: "Plan a tour during a test stay", href: "/guides/how-to-plan-an-empathy-school-tour-during-a-test-stay" });
  }
  ...
  if (move === "direct-move") {
    ...
    watchouts.push("A direct move gets noisier when housing hardens before school and commute reality are grounded.");
  }
  ...
  if (commute === "far-is-fine") {
    ...
    watchouts.push("Parents often tolerate the drive more easily than the child, especially by week two.");
  }

  if (commute === "unknown") {
    add(5,
      "Treat the commute as part of the decision, not a side note."
    );
  }
  ...
  let summary = "You have enough signal to use the school as a real decision anchor — especially for area, commute, and weekly rhythm — without pretending the answer is already final.";

  if (score >= 42) {
    band = "Strong anchor";
    headline = "Let Empathy School influence the move early enough to matter.";
    summary = "Your answers suggest school fit is not a side note. Use it to pressure-test area, commute, and whether the week still feels calm once normal life begins.";
  } else if (score >= 32) {
    band = "Test during a stay";
    headline = "Bring the school question forward, but keep it tied to real family days.";
    summary = "The fit question looks meaningful, but it will become much clearer inside a test stay, a tour, and a realistic view of the commute.";
  } else {
    band = "Conversation before commitment";
    headline = "Keep Empathy School in the picture, but do not make it carry the whole move yet.";
    summary = "Your answers suggest you need a calmer first step: a direct conversation, a sharper shortlist, or a more honest view of the week before school becomes the main anchor.";
  }

  if (stage === "older") {
    band = "Conversation first";
    headline = "Start with a direct fit conversation before you let school anchor the move.";
    summary = "Because the public materials on this site currently focus on Early Years through Middle School, the cleanest next step is a direct conversation about whether Empathy School is even the right active question for your family.";
  }
  ...
  nextLinks.push({ title: "Ask about Empathy School fit", href: buildContactHref("Empathy School fit", { from: "/empathy-school-fit" }) });
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[0.95fr_1.05fr]">
      <div className={cardCls}>
        ...
        <h2 className="mt-4 text-2xl font-semibold tracking-tight text-gray-900">Use school fit as a real move decision</h2>
        <p className="mt-3 text-sm leading-6 text-gray-600">
          This tool does not tell you whether Empathy School is "the answer." It helps you judge whether the school question deserves to move forward now — and whether it should shape area, commute, and the weekly rhythm early enough to matter.
        </p>
        ...
        <div className="mt-8 grid gap-5">
          ...
          <label className="grid gap-2">
            <span className="text-sm font-medium text-gray-900">What matters most</span>
            <select className={inputBase} value={priority} onChange={(e) => setPriority(e.target.value as PriorityKey)}>
          ...
          <label className="grid gap-2">
            <span className="text-sm font-medium text-gray-900">School-run tolerance</span>
          ...
          <label className="grid gap-2">
            <span className="text-sm font-medium text-gray-900">What you hope school will help with</span>
            <select className={inputBase} value={solve} onChange={(e) => setSolve(e.target.value as SolveKey)}>
        ...
        <div className="mt-6 rounded-2xl border border-gray-200 bg-gray-50 p-4">
          <div className="text-sm font-semibold text-gray-900">What this is trying to prevent</div>
          <p className="mt-2 text-sm leading-6 text-gray-600">
            The common mistake is judging school as an isolated product. Families usually make better decisions when they judge campus feel, commute, area logic, and child energy as one combined reality.
          </p>
        </div>

        <div className={btnRow}>
          <a className={buttonPrimary} href="/schools" data-track="fit_builder_open_school">
            Explore Empathy School
          </a>
          <a className={buttonSecondary} href="/empathy-school-tour-prep" data-track="fit_builder_tour_prep">
            Plan the tour day
          </a>
        </div>
      </div>
      ...
      <div className="grid gap-6">
        <div className={cardCls}>
          ...
          <h3 className="mt-4 text-2xl font-semibold tracking-tight text-gray-900">{result.headline}</h3>
          <p className="mt-3 text-sm leading-6 text-gray-600">{result.summary}</p>
        </div>

        <div className="grid gap-4 md:grid-cols-2">
          <div className={cardCls}>
            <h3 className="text-xl font-semibold tracking-tight text-gray-900">Signals pointing toward a real fit test</h3>
          ...
          <div className={cardCls}>
            <h3 className="text-xl font-semibold tracking-tight text-gray-900">Watch carefully</h3>
            <ul className="mt-4 list-disc space-y-3 pl-5 text-sm leading-6 text-gray-600">
