...
export default function Navbar() {
  ...
  const linkCls = (href: string) =>
    cn(
      "rounded-lg px-2 py-1 text-sm font-medium text-gray-600 transition hover:text-gray-900",
      ...
      <div className="container px-4 mx-auto">
        <div className="flex items-center justify-between gap-4 py-4">
          <Link className="flex items-center gap-3 font-semibold tracking-tight text-gray-900" href="/" data-track="nav_home">
            <span className="h-2.5 w-2.5 rounded-full bg-gradient-to-br from-blue-500 to-blue-700 shadow-sm" aria-hidden />
          ...
          <div className="flex items-center gap-3">
            ...
            <nav className="hidden items-center gap-1 md:flex" aria-label="Primary navigation">
              ...
              <div className="relative" ref={otherRef}>
                <button
                  type="button"
                  className={cn(
                    "flex items-center gap-1.5 rounded-lg px-2 py-1 text-sm font-medium text-gray-600 transition hover:text-gray-900",
                    otherOpen && "text-gray-900"
                  )}
                  onClick={() => setOtherOpen((v) => !v)}
                >
                  Other
                  <ChevronDownIcon className={cn("h-4 w-4 transition", otherOpen ? "rotate-180" : "rotate-0")} />
                </button>

                <div className={cn(
                  "absolute right-0 mt-2 w-56 origin-top-right rounded-2xl border border-gray-100 bg-white p-2 shadow-lg transition",
                  otherOpen ? "opacity-100 scale-100" : "pointer-events-none opacity-0 scale-95"
                )}>
                  {other.map((item) => (
                    <Link key={item.href} href={item.href} className="block rounded-xl px-3 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 hover:text-gray-900" onClick={() => setOtherOpen(false)}>
                      {item.label}
                    </Link>
                  ))}
                </div>
            ...
            {/* Primary CTA - SEKARANG BENAR-BENAR TERSEMBUNYI DI MOBILE */}
            <Link 
        ...
        <div className={cn(
          ...
          <nav className="grid gap-1 pt-2">
            {navItems.map((item) => (
              <Link key={item.href} href={item.href} className="rounded-xl px-3 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50" onClick={() => setMobileOpen(false)}>
                {item.label}
              </Link>
            ))}
          </nav>
          ...
          <div className="mt-4 px-1">
            <Link
              href={site.ctas.empathy.href}
              className={cn(buttonPrimary, "w-full text-center flex justify-center")}
              data-track="nav_empathy_mobile"
            >
              Empathy School
            </Link>
          </div>
